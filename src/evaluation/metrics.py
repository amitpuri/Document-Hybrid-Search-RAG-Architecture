...
def is_relevant(
    ...
    # TODO (P3): This substring fallback `target_doc.lower() in chunk_text[:120]` is
    # fragile — a document filename appearing in the first 120 chars of an unrelated
    # chunk could produce a false positive. An audit of dataset.py confirms all 14
    # current benchmark queries specify an explicit target_chunk_idx, so this branch
    # is currently unreachable. Future: replace with warnings.warn() so any regression
    # to the fallback path is immediately visible rather than silently passing.
...
def evaluate_ranking(
    ...
    """
    relevance_flags = [
        is_relevant(corpus_texts[idx], idx, target_doc, target_chunk_idx) for idx in ranked_indices
    ]
