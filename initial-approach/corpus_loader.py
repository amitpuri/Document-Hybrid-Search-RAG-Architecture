...
# ===========================================================
def _compute_cache_key(corpus_dir, max_words, overlap_sentences):
    """
    Computes a cryptographic hash over corpus files (name, mtime, size) and chunking params.
    ...
    hasher.update(f"max_words:{max_words},overlap_sents:{overlap_sentences}".encode("utf-8"))
...
def load_structured_corpus(corpus_dir, cache_dir=".cache", force_rebuild=False, max_words=120, overlap_sentences=1):
    ...
    """
    cache_key = _compute_cache_key(corpus_dir, max_words, overlap_sentences)
