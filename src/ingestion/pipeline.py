...
_CHUNK_PREFIX_REGEX = re.compile(
    r"^\[(?P<doc>[^|]+)\s*\|\s*Page\s*(?P<page>\d+)\s*\|\s*§\s*(?P<section>[^\]]+)\]\s*(?P<text>.*)$",
    re.DOTALL,
)
...
class IngestionPipeline:
    ...
    def _run_parquet(self, force_rebuild: bool = False) -> Tuple[ParquetChunkStore, int, List[str]]:
        ...
        if manifest is not None:
            ...
            if saved_hash and saved_hash != current_regime_hash:
                raise IngestionRegimeMismatchError(
                    f"Cannot incrementally ingest into dataset with mismatched chunking parameters. "
                    f"Dataset regime: {saved_regime}; "
                    f"Requested: max_words={self.max_words}, overlap_sentences={self.overlap_sentences}. "
                    f"Specify force_rebuild=True to re-chunk the entire corpus uniformly, or use matching parameters."
                )
    ...
    def _run_memory(self, force_rebuild: bool = False) -> Tuple[InMemoryChunkStore, int, List[str]]:
        ...
        if not force_rebuild and manifest is not None and self.dataset_dir.exists():
            ...
            pdf_paths = sorted([str(p) for p in self.corpus_dir.glob("*.pdf")])
            return InMemoryChunkStore(chunks), manifest.get("total_pages", 0), pdf_paths
        ...
        if not force_rebuild and cache_file.exists():
            ...
            if cached_data is not None:
                ...
                return InMemoryChunkStore(chunks), total_pages, pdf_paths
