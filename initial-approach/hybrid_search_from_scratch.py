...
def extract_text_from_pdf(pdf_path):
    ...
    Supports pypdfium2, pdfplumber, or PyPDF2 based on availability.
    """
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(f"PDF file not found: {pdf_path}")

    # Try pypdfium2 first (clean text, preserves whitespace)
    try:
...
def load_pdf_corpus(pdf_inputs, chunk_size=120, overlap=30):
    """
    Loads one or more PDFs and chunks their content into passages with page and document metadata.
    Accepts either a single PDF path or a list/array of PDF paths.
    """
    ...
    for pdf_path in pdf_inputs:
        ...
        for page_num, text in pages:
            ...
            for i in range(0, len(words), step):
                ...
                if len(chunk_words) >= 25:  # skip tiny trailing fragments
