...
class BaseGraphStore(ABC):
    ...
    @classmethod
    @abstractmethod
    def load(cls, path: Union[Path, str]) -> "BaseGraphStore":
...
class NetworkXGraphStore(BaseGraphStore):
    ...
    def get_node_degree(self, name: str) -> int:
        if self.graph.has_node(name):
            return int(self.graph.degree(name))
        return 0
    ...
    @classmethod
    def load(cls, path: Union[Path, str]) -> "NetworkXGraphStore":
        ...
        instance._communities_cache = None

        for ent_dict in data.get("entities", []):
            instance.add_entity(Entity.from_dict(ent_dict))

        for rel_dict in data.get("relationships", []):
            instance.add_relationship(Relationship.from_dict(rel_dict))
