...
6. **`eval_harness.py`**: Quantitative IR evaluation framework. Benchmarks **9 strategies** across 10 ground-truth queries measuring MRR, Recall@K, and NDCG@5.
...
### 2. Structure- & Sentence-Aware Chunking (`structure_aware_chunking.py`)

Legacy standalone entry-point; internally delegates to `corpus_loader.py`.

- **Sentence-Boundary Packing**: Avoids splitting across clauses, equations, or citations.
- **Section Heading Inheritance**: Detects hierarchical section titles and prepends them to each chunk.

```bash
python structure_aware_chunking.py
```

### 3. Reciprocal Rank Fusion & MMR Diversity (`hybrid_search_rrf.py`)
...
### 4. Distributional Semantic Search From Scratch (`pmi_semantic_search_from_scratch.py`)
...
### 5. IR Evaluation Harness (`eval_harness.py`)

Quantitative benchmarking framework comparing **9 retrieval strategies** across **10 curated ground-truth queries**.

**Design decisions:**
- **Relevance signal**: checks whether the target PDF filename appears in the chunk's 120-char structured prefix (e.g. `[2605.23950v1.pdf | Page 3 | § ...]`). This is robust to paraphrase and avoids brittle key-phrase substring matching.
- **Single dispatch path**: all 9 strategies are built into a `ranked_by_strategy` dict per query — no dead lambda definitions.
- **PPMI model built once** before the query loop (~20–40 s) and reused across all queries.

**Metrics:** MRR, Recall@1/3/5, NDCG@5.

```bash
python eval_harness.py
```

#### Empirical Benchmark — 10 queries × 9 strategies (11 PDFs, 354 pages, 2072 chunks)

| Retrieval Strategy | MRR | Recall@1 | Recall@3 | Recall@5 | NDCG@5 |
|---|---|---|---|---|---|
| **1. Pure BM25 (Sparse)** | **0.933** | 0.900 | **1.000** | **1.000** | 2.547 |
| 2. Pure TF-IDF (Dense) | 0.867 | 0.800 | 0.900 | 0.900 | 2.422 |
| 3. Linear Hybrid (α=0.3) | **0.933** | 0.900 | **1.000** | **1.000** | 2.547 |
| 4. Linear Hybrid (α=0.5) | 0.925 | 0.900 | 0.900 | **1.000** | 2.541 |
| 5. Linear Hybrid (α=0.7) | 0.925 | 0.900 | 0.900 | **1.000** | 2.541 |
| 6. RRF (k=60) | 0.925 | 0.900 | 0.900 | **1.000** | 2.541 |
| 7. RRF + Deduplication | 0.925 | 0.900 | 0.900 | **1.000** | 2.541 |
| **8. RRF + Dedup + MMR** | **0.933** | 0.900 | **1.000** | **1.000** | 2.547 |
| 9. PPMI Semantic + BM25 RRF | 0.883 | 0.800 | **1.000** | **1.000** | **2.597** |

**Key observations:**
- **BM25 is very strong** on title-heavy queries where query tokens appear verbatim in paper title/abstract chunks.
- **RRF + Dedup + MMR** matches the BM25 MRR peak (0.933) while additionally diversifying the result set — best for multi-facet queries.
- **PPMI** achieves the **highest NDCG@5 (2.597)**, meaning it ranks more relevant chunks in the top-5 than any keyword method, even though its rank-1 accuracy is slightly lower. It recovers semantically related passages that exact-token methods miss.
- **TF-IDF alone** is the weakest overall (MRR 0.867, Recall@5 0.900).
