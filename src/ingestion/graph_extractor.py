...
_ACADEMIC_AFFILIATION_WORDS = re.compile(
    r"\b(?:University|College|Department|Institut(?:e|o)|School\s+of|Faculty\s+of|"
    r"Laboratory|Lab\s+of|Center\s+for|Centre\s+for|Division\s+of|Program\s+in|"
    r"Graduate\s+School|Foundation|Corporation|Research\s+Group)\b",
    re.IGNORECASE,
)
...
_SECTION_ARTIFACT_RE = re.compile(
    ...
    r"Introduction|Conclusion|References|Acknowledgm|Appendix|Related Work)",
    re.IGNORECASE,
)
...
def _is_likely_noise(name: str, entity_type: str, num_chunks: int) -> bool:
    ...
    # Drop entities containing digits mixed into word (section title artifacts like "1 Introduction")
    ...
    def extract_from_chunks(
        ...
        for chunk in chunks:
            ...
            if section and section.lower() not in {
                "overview", "abstract", "introduction", "references",
                "conclusion", "acknowledgments", "appendix", "related work",
            }:
        ...
        for (src, tgt), info in pair_counts.items():
            if src in valid_entity_names and tgt in valid_entity_names and src != tgt:
                ...
                if len(desc) > 200:
                    desc = desc[:197] + "..."
                # Log-scale cap: prevents boilerplate pairs (count=50) from dominating low (count=2) pairs
...
{
  "entities": [
    {"name": "string", "entity_type": "CONCEPT|TECHNICAL_ACRONYM|FRAMEWORK_COMPONENT|MODEL|METRIC|ALGORITHM", "description": "short description"}
  ],
  "relationships": [
    {"source": "entity_name", "target": "entity_name", "relation_type": "USES|EXTENDS|EVALUATES|CONSTRAINS|IMPLEMENTS|RELATED_TO", "description": "brief context", "weight": 1.0}
  ]
}

Rules:
- Only extract domain-specific technical entities (algorithms, frameworks, models, metrics, formal concepts).
- Do NOT extract author names, affiliations, section numbers, or generic English phrases.
...
class LLMGraphExtractor(BaseGraphExtractor):
    ...
    def __init__(self, generator=None, fallback_on_error: bool = True, batch_size: int = 1):
    ...
    @staticmethod
    def _parse_response(raw: str, chunk_id: int, doc_name: str) -> Tuple[List[Entity], List[Relationship]]:
